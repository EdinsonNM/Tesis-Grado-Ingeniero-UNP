...
**Mem0**. Sistema de memoria adaptativa para agentes LLM que combina recuperación vectorial con grafos de conocimiento para gestionar memorias de usuario, agente y sesión de forma diferenciada. Diseñado para entornos de producción con múltiples usuarios y sesiones concurrentes. *([Chhikara et al., 2025](https://arxiv.org/abs/2504.19413))*

**MemGPT / Letta**. Sistema de gestión de memoria para agentes LLM que simula el comportamiento de un sistema operativo, con memoria principal (contexto activo) y memoria secundaria (almacenamiento externo). Implementa técnicas de paginación de memoria para superar la limitación de la ventana de contexto y fue el precursor del concepto de SessionFacts. *([Packer et al., 2023](https://arxiv.org/abs/2310.08560))*

**MemoryBank**. Sistema de memoria a largo plazo para agentes LLM que combina almacenamiento externo indexado con una política de olvido inspirada en la curva de Ebbinghaus de la psicología cognitiva. Las memorias almacenadas decaen en relevancia con el tiempo transcurrido desde su última recuperación, evitando la acumulación de información obsoleta. *([Hu et al., 2023](https://arxiv.org/abs/2305.10250))*

**Model Context Protocol *(MCP)***. Estándar abierto publicado por Anthropic en noviembre de 2024 para la integración de agentes LLM con herramientas y fuentes de datos externas mediante una interfaz cliente-servidor estandarizada. Define los mecanismos de inicialización de sesión, descubrimiento de herramientas, invocación y notificación de cambios en el catálogo, siendo el protocolo de integración de herramientas de referencia para la investigación. *([Anthropic, 2024](https://modelcontextprotocol.io/specification))*

**Modelo de lenguaje de gran escala *(LLM)***. Sistema de inteligencia artificial basado en la arquitectura transformer, entrenado sobre vastos corpus de texto mediante el objetivo de predicción del siguiente token. Opera dentro de una ventana de contexto limitada que define el número máximo de tokens que puede procesar en una inferencia. Los LLM modernos exhiben capacidades emergentes de razonamiento, uso de herramientas y seguimiento de instrucciones complejas. *([Brown et al., 2020](https://proceedings.neurips.cc/paper/2020/hash/1457c0d6bfcb4967418bfb8ac142f64a-Abstract.html))*
